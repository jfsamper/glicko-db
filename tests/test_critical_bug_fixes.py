...
class TestCSVImportConnectionLeak:
    """Tests for CSV import connection leak fix."""

    def test_csv_import_connection_closes_on_exception(self, tmp_path, monkeypatch):
        """Verify that database connection closes even when exception occurs."""
        ...
        monkeypatch.setattr(common, "get_db", mock_get_db)

        # Simulate CSV processing with connection management
        exception_caught = False
        conn = mock_get_db()
        try:
            # Simulate an error during processing
            raise ValueError("Test error during CSV import")
        except ValueError:
            exception_caught = True
        finally:
            conn.close()

        # Verify exception was caught and connection was closed
        assert exception_caught

        # After finally block, connection should be closed
        # Verify by checking we can get a fresh connection
        new_conn = mock_get_db()
    ...
    def test_csv_import_commits_on_success(self, tmp_path, monkeypatch):
        """Verify that successful import commits data."""
        import services.common as common
        ...
        # Simulate successful import with try-finally
        conn = mock_get_db()
        try:
            # Insert data
            conn.execute(
        ...
        verify_conn = mock_get_db()
        player = verify_conn.execute(
            "SELECT * FROM players WHERE display_name = ?", ("Test Player",)
        ).fetchone()
