...
def build_smooth_path(points):
    ...
    for index in range(1, len(points)):
        ...
        if index == len(points) - 1:
            control_one_x = previous["x"] + (current["x"] - previous["x"]) * 0.5
            ...
            control_one_x = previous["x"] + (current["x"] - previous["x"]) * 0.5
            control_one_y = previous["y"]
            control_two_x = current["x"] - (next_point["x"] - previous["x"]) * 0.25
            control_two_y = current["y"]
        parts.append(
...
def build_rating_chart_data(snapshots):
    ...
    for index, row in enumerate(snapshots):
        if len(snapshots) == 1:
            ...
            x = padding + (index / (len(snapshots) - 1)) * (width - padding * 2)
        value = row["rating"]
        y = height - padding - ((value - min_rating) / span) * (height - padding * 2)
        points.append(
            {
                "x": round(x, 2),
                "y": round(y, 2),
    ...
    baseline_y = height - padding - ((initial_rating - min_rating) / span) * (height - padding * 2)
