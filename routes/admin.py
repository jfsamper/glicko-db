    ...
    if request.method == "POST":
        ...
        if not valid:
            ...
            conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (match_date.strip(), w_id, b_id, result, event, notes, normalize_round_note(raw_notes)),
            )
            conn.commit()
            refresh_stats()
        ...
        if not valid:
            ...
            try:
                conn.execute(
                    ...
                    WHERE id = ?
                    """,
                    (match_date.strip(), w_id, b_id, result, event, notes, normalize_round_note(raw_notes), match_id),
                )
                conn.commit()
                refresh_stats()
