...
def admin_matches():
    ...
    try:
        ...
        page_size = int(request.args.get("page_size")) if request.args.get("page_size") is not None else 25
    ...
    filter_sql, filter_params = admin._match_filter_sql(date_from, date_to, player_id)
    conn = admin.get_db()
    admin.ensure_sgf_schema(conn)
    admin.clear_missing_sgf_links(conn)
    total_count = conn.execute(f"SELECT COUNT(*) FROM matches m {filter_sql}", filter_params).fetchone()[0]
    ...
    match_players = conn.execute("SELECT id, display_name FROM players ORDER BY display_name").fetchall()
    conn.close()
...
def _match_form_data(admin, conn, lang):
    ...
    valid, message = admin.validate_match_form_data(conn, match_date, white_player_id, black_player_id, result, lang)
        ...
        try:
            ...
            if sgf_file and sgf_file.filename:
                sgf_filename = admin.save_sgf_upload(
                    sgf_file,
                    admin.match_sgf_metadata(conn, white_player_id, black_player_id, match_date, result, event, location=location),
                )
...
def admin_add_match():
    ...
    if request.method == "POST":
        ...
        if valid:
            ...
            conn.execute("INSERT INTO matches (match_date, white_player_id, black_player_id, result, event, location, notes, round_number, handicap_stones, sgf_filename) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (match_date, white_id, black_id, result, event, location, notes, admin.normalize_round_note(raw_notes), handicap, sgf))
            conn.commit()
            admin.refresh_stats(); admin.mark_dirty(match_date); admin.update_from_latest_snapshot()
            admin.log_admin_action("match_created", "match", {"match_id": conn.execute("SELECT last_insert_rowid()").fetchone()[0], "event": event or None}, user_id=admin.session.get("user_id"))
            flash(admin.TRANSLATIONS[lang]["success"]); conn.close()
            return redirect(url_for("admin_matches", lang=lang))
        flash(message or admin.TRANSLATIONS[lang]["error"])
    players = conn.execute("SELECT id, display_name FROM players ORDER BY display_name").fetchall(); conn.close()
...
def admin_edit_match():
    ...
    if not admin.admin_required():
        return redirect(url_for("admin_login", lang=admin.get_language(request.args.get("lang"))))
    lang = admin.get_language(request.args.get("lang")); match_id = request.args.get("id")
    if not match_id:
        flash(admin.TRANSLATIONS[lang]["error"]); return redirect(url_for("admin_matches", lang=lang))
    conn = admin.get_db(); admin.ensure_sgf_schema(conn); admin.clear_missing_sgf_links(conn)
    match = conn.execute("SELECT * FROM matches WHERE id = ?", (match_id,)).fetchone()
    if not match:
        conn.close(); flash(admin.TRANSLATIONS[lang]["error"]); return redirect(url_for("admin_matches", lang=lang))
    if request.method == "POST":
        valid, message, data = _match_form_data(admin, conn, lang)
        if valid:
            ...
            if not new_sgf:
                new_sgf = None if request.form.get("remove_sgf") == "1" else old_sgf
            try:
                conn.execute("UPDATE matches SET match_date = ?, white_player_id = ?, black_player_id = ?, result = ?, event = ?, location = ?, notes = ?, round_number = ?, handicap_stones = ?, sgf_filename = ? WHERE id = ?", (match_date, white_id, black_id, result, event, location, notes, admin.normalize_round_note(raw_notes), handicap, new_sgf, match_id))
                admin.sync_match_pairing(conn, match_id, white_id, black_id, result, handicap); conn.commit()
                if new_sgf:
                    admin.update_sgf_metadata(new_sgf, admin.match_sgf_metadata(conn, white_id, black_id, match_date, result, event))
                admin.refresh_stats(); admin.mark_dirty(match["match_date"]); admin.mark_dirty(match_date); admin.update_from_latest_snapshot()
                admin.log_admin_action("match_updated", "match", {"match_id": match_id}, user_id=admin.session.get("user_id")); flash(admin.TRANSLATIONS[lang]["success"])
            except ValueError as exc:
                conn.rollback(); flash(f"{admin.TRANSLATIONS[lang]['error']}: {exc}")
            except sqlite3.DatabaseError as exc:
                conn.rollback()
                admin.logger.exception("Match %s was saved, but post-save statistics refresh failed; restoring backup.", match_id)
...
def admin_delete_match():
    ...
    if not match_id:
        flash(admin.TRANSLATIONS[lang]["error"]); return redirect(url_for("admin_matches", lang=lang))
    conn = admin.get_db(); admin.ensure_sgf_schema(conn); admin.clear_missing_sgf_links(conn)
    row = conn.execute("SELECT match_date, sgf_filename FROM matches WHERE id = ?", (match_id,)).fetchone()
    if not row:
        conn.close(); flash(admin.TRANSLATIONS[lang]["error"]); return redirect(url_for("admin_matches", lang=lang))
    conn.execute("DELETE FROM matches WHERE id = ?", (match_id,)); conn.commit(); conn.close()
    admin.log_admin_action("match_deleted", "match", {"match_id": match_id}, user_id=admin.session.get("user_id"))
    try:
        admin.refresh_stats(); admin.mark_dirty(row["match_date"]); admin.update_from_latest_snapshot()
    except sqlite3.DatabaseError as exc:
        admin.logger.exception("Match deletion for %s left the database in a malformed state; restoring a valid backup.", match_id)
        backup_path = admin.get_latest_valid_backup_path()
        if backup_path is not None and admin.restore_db_from_backup(backup_path):
            flash("Database recovered from backup after a malformed SQLite image. Please review the restored data before continuing.")
        else:
            flash(f"{admin.TRANSLATIONS[lang]['error']}: {exc}")
    else: flash(admin.TRANSLATIONS[lang]["success"])
    return redirect(url_for("admin_matches", lang=lang))
